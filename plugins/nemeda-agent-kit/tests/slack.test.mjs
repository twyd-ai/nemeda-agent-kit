...
test("classifyEvent answers a mention from the owner and a guest", () => {
  for (const user of ["U01OWNER", "U02GUEST"]) {
    const decision = classifyEvent({
      event: { type: "app_mention", user, text: "<@U09BOTXX> what does orders do?", channel: "C01ABCDEF", ts: "1.0" },
      botUserId: "U09BOTXX",
      route
    });
    assert.equal(decision.action, "answer");
    assert.equal(decision.question, "what does orders do?");
  }
});

test("classifyEvent denies a stranger and ignores bots, subtypes, and itself", () => {
  const deny = classifyEvent({ event: { type: "app_mention", user: "U03OTHER", text: "<@U09BOTXX> hello", channel: "C01ABCDEF", ts: "1.0" }, botUserId: "U09BOTXX", route });
...
test("classifyEvent follows a known thread without a new mention, but ignores unrelated chatter", () => {
  const event = { type: "message", user: "U01OWNER", text: "and the rate limit?", channel: "C01ABCDEF", ts: "3.0", thread_ts: "2.0" };
  ...
  const loose = { type: "message", user: "U01OWNER", text: "good morning", channel: "C01ABCDEF", ts: "4.0" };
...
test("classifyEvent reports an unrouted channel instead of guessing", () => {
  const decision = classifyEvent({ event: { type: "app_mention", user: "U01OWNER", text: "<@U09BOTXX> hello", channel: "C99ZZZZZZ", ts: "1.0" }, botUserId: "U09BOTXX", route: null });
  assert.equal(decision.action, "unrouted");
});

test("stripMentions removes every user and bot mention", () => {
  assert.equal(stripMentions("<@U09BOTXX> look at <@U123> this"), "look at this");
});
...
test("toMrkdwn converts Markdown but leaves code fences alone", () => {
  // Deliberately accented: the transforms are regex-based and must not mangle
  // non-ASCII text, which most answers in this workspace contain.
  const converted = toMrkdwn("## Título\n\n**negrita** and [link](https://x.com)\n\n- one\n- two");
  assert.equal(converted, "*Título*\n\n*negrita* and <https://x.com|link>\n\n• one\n• two");
...
test("buildBackendCommand stays read-only and resumes an existing thread", () => {
  const fresh = buildBackendCommand(route, { question: "and this?", cwd: "/repo", sessionId: "abc", resume: false, prompt: "voice" });
  ...
  const resumed = buildBackendCommand(route, { question: "and?", cwd: "/repo", sessionId: "abc", resume: true, prompt: "voice" });
...
test("buildBackendCommand sandboxes the codex backend", () => {
  ...
  const command = buildBackendCommand(codexRoute, { question: "and this?", cwd: "/repo", sessionId: "abc", prompt: "voice" });
...
test("parseBackendOutput unwraps claude json, surfaces its error, and survives plain text", () => {
  assert.deepEqual(parseBackendOutput("claude", JSON.stringify({ result: " hi " })), { text: "hi", error: null });
  ...
  assert.equal(parseBackendOutput("codex", " answer ").text, "answer");
...
test("resolveDmRoute picks by switch, prefix, stickiness, and only-one", () => {
  ...
  assert.equal(resolveDmRoute({ text: "use milence", projects }).kind, "switch");
  assert.equal(resolveDmRoute({ text: "use Nemeda Agent Kit", projects }).route.projectId, "agent-kit");
  const prefixed = resolveDmRoute({ text: "agent-kit: what does doctor do?", projects });
  assert.equal(prefixed.route.projectId, "agent-kit");
  assert.equal(prefixed.question, "what does doctor do?");
  assert.equal(resolveDmRoute({ text: "hello", projects }).kind, "choose");
  assert.equal(resolveDmRoute({ text: "hello", projects, activeProjectId: "milence" }).route.projectId, "milence");
  assert.equal(resolveDmRoute({ text: "use scharlab", projects }).kind, "unknown-project");
  assert.equal(resolveDmRoute({ text: "anything at all", projects: [projects[0]] }).route.projectId, "milence");
});

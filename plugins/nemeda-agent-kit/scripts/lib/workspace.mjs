...
function validateMeetings(meetings, issues) {
  ...
  validateAllowedKeys(meetings, ["inbox", "transcripts", "notes", "language", "naming", "memory", "knowledgeLog", "recordings"], "meetings", issues);
  ...
  if (meetings.memory !== undefined && typeof meetings.memory !== "boolean") {
    issues.push({ level: "error", code: "invalid-meetings", message: "meetings.memory must be a boolean." });
  }
  if (meetings.knowledgeLog !== undefined) {
    if (typeof meetings.knowledgeLog !== "boolean") {
      issues.push({ level: "error", code: "invalid-meetings", message: "meetings.knowledgeLog must be a boolean." });
    } else {
      issues.push({ level: "warn", code: "deprecated-knowledge-log", message: "meetings.knowledgeLog is deprecated and ignored; meetings are logged to the memory section (meetings.memory, default true)." });
    }
  }
  if (meetings.recordings !== undefined) {
    if (!isObject(meetings.recordings) || !["local", "archive", "delete"].includes(meetings.recordings.keep)) {
      ...
      validateAllowedKeys(meetings.recordings, ["keep", "afterDays", "path"], "meetings.recordings", issues);
      ...
      if (meetings.recordings.keep === "archive" && !isRelativeInsidePath(meetings.recordings.path)) {
        issues.push({ level: "error", code: "invalid-meetings", message: "meetings.recordings.path must be a relative folder inside the workspace when keep is archive." });
      }
    }
  }
}
...
export function formatContextForHook(context) {
  ...
  if (context.config.meetings) {
    ...
    lines.push(`Meetings: the kit files one folder per transcribed recording under ${meetings.transcripts}/ (transcript.txt, transcript.srt, transcript.json, meta.json)${meetings.notes ? `; meeting notes (Summary, Decisions, Action items, Open questions) go to ${meetings.notes}/ and are generated by the kit` : ""}${meetings.memory === false ? "" : "; every transcribed meeting is logged to the project memory"}. New recordings are transcribed with \`nemeda-agent meeting process\`; never transcribe or copy them by hand. For a transcript without notes, run the meeting-notes skill or \`nemeda-agent meeting notes FOLDER\`.`);
  }
